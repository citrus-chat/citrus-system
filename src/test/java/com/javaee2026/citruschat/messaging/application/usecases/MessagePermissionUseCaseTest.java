...
class MessagePermissionUseCaseTest {
	...
	private final ChatRoomId chatRoomId = new ChatRoomId(UUID.fromString("eeeeeeee-0000-0000-0000-000000000101"));
	private final UserId creatorUserId = new UserId(UUID.fromString("eeeeeeee-0000-0000-0000-000000000102"));
	private final UserId requesterUserId = new UserId(UUID.fromString("eeeeeeee-0000-0000-0000-000000000103"));
	private final RoleId viewRoleId = new RoleId(UUID.fromString("eeeeeeee-0000-0000-0000-000000000104"));
	private final RoleId sendRoleId = new RoleId(UUID.fromString("eeeeeeee-0000-0000-0000-000000000105"));
	...
	void participantWithOnlyViewMessagePermissionCanSyncMessages() {
		ChatRoom room = room(role(viewRoleId, "VIEWER", ChatPermissionList.CAN_VIEW_MESSAGE));
		...
		when(messageRepository.findMessagesAfter(chatRoomId, null, 100)).thenReturn(List.of());

		assertDoesNotThrow(() -> syncUseCase().execute(new SyncMessagesCommand(chatRoomId, requesterUserId, null)));
	...
	void participantWithoutViewMessagePermissionCannotSyncMessages() {
		ChatRoom room = room(role(sendRoleId, "SENDER", ChatPermissionList.CAN_SEND_MESSAGE));
		when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
		effectiveRequesterPermissions(ChatPermissionList.CAN_SEND_MESSAGE);

		assertThrows(ChatPermissionDeniedException.class,
				() -> syncUseCase().execute(new SyncMessagesCommand(chatRoomId, requesterUserId, null)));
	...
	void participantWithViewMessagePermissionCanListMessages() {
		...
		when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
		effectiveRequesterPermissions(ChatPermissionList.CAN_VIEW_MESSAGE);
		when(messageRepository.findMessagesByChatRoomId(chatRoomId, 0, 100)).thenReturn(List.of());

		assertDoesNotThrow(() -> getMessagesUseCase().execute(chatRoomId.value(), requesterUserId.value(), -1, 500));
	...
	void participantWithoutViewMessagePermissionCannotListMessages() {
		...
		assertThrows(ChatPermissionDeniedException.class,
				() -> getMessagesUseCase().execute(chatRoomId.value(), requesterUserId.value(), 0, 50));

		verify(messageRepository, never()).findMessagesByChatRoomId(chatRoomId, 0, 50);
	}
	...
	void participantWithOnlyViewMessagePermissionCannotSendMessages() {
		...
		assertThrows(ChatPermissionDeniedException.class, () -> sendUseCase().validateSenderPermissions(room, sender));
	}
	...
	void participantWithSendMessagePermissionCanSendMessages() {
		...
		assertDoesNotThrow(() -> sendUseCase().validateSenderPermissions(room, sender));
	}
	...
	private GetChatRoomMessagesUseCase getMessagesUseCase() {
		return new GetChatRoomMessagesUseCase(messageRepository, authorizationService());
	}
	...
	private void effectiveRequesterPermissions(String permissionCode) {
		ChatParticipant participant = participant(
				ChatPermissionList.CAN_VIEW_MESSAGE.equals(permissionCode) ? viewRoleId : sendRoleId);
		when(chatParticipantRepository.findActiveByChatRoomIdAndUserId(chatRoomId, requesterUserId))
				.thenReturn(Optional.of(participant));
		when(chatPermissionRepository.findPermissionsByChatRoomAndParticipant(chatRoomId, participant.getId()))
	...
	private ChatRoom room(ChatRole role) {
		return new ChatRoom(chatRoomId, ChatRoomType.GROUP, "Group", creatorUserId, List.of(participant(role.getId())),
				Map.of(role.getName(), role), Instant.now(), null, null);
	}

	private ChatParticipant participant(RoleId roleId) {
		return ChatParticipant.reconstitute(new ParticipantId(UUID.fromString("eeeeeeee-0000-0000-0000-000000000106")),
	...
	private ChatRole role(RoleId roleId, String name, String permissionCode) {
		return ChatRole.reconstitute(roleId, chatRoomId, Set.of(permission(permissionCode)), name, 10, Instant.now());
	}
