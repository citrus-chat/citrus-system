...
// mock(IChatRoomRepository.class);
//
...
// mock(IChatPermissionRepository.class);
//
// private final CreateChatRoomUseCase useCase = new
// CreateChatRoomUseCase(chatRoomRepository, new ChatRoomFactory(),
...
//
// CreateChatRoomCommand command = new CreateChatRoomCommand(ChatRoomType.GROUP,
// "Test Group", creatorId,
...
// CreateChatRoomCommand(ChatRoomType.DIRECT, "Direct Chat", creatorId,
...
// () -> useCase.execute(command));
//
// assertEquals("Direct chat rooms must have exactly one participant",
...
// UUID creatorId = UUID.randomUUID();
//
// CreateChatRoomCommand command = new CreateChatRoomCommand(null, "Test Group",
// creatorId,
...
// p.getUserId().value().equals(creatorId)));
//
// assertTrue(participants.stream().anyMatch(p ->
// p.getUserId().value().equals(participantId1)));
//
// assertTrue(participants.stream().anyMatch(p ->
// p.getUserId().value().equals(participantId2)));
// }
//
// @Test
...
//
// CreateChatRoomCommand command = new
// CreateChatRoomCommand(ChatRoomType.DIRECT, "Private Chat", creatorId,
// List.of(participantId));
//
// when(userRepository.findById(any(UserId.class))).thenReturn(Optional.of(mock(User.class)));
//
...
//
// assertTrue(participants.stream().anyMatch(p ->
// p.getUserId().value().equals(participantId)));
// }
//
// @Test
...
// .filter(p ->
// p.getUserId().value().equals(creatorId)).findFirst().orElse(null);
//
...
// when(permissionRepository.findByCodes(anySet())).thenReturn(Set.of(mock(ChatPermission.class)));
//
// when(chatRoomRepository.existsDirectChatBetweenParticipants(new
// UserId(creatorId), new UserId(participantId)))
// .thenReturn(true);
//
// IllegalArgumentException exception =
// assertThrows(IllegalArgumentException.class,
// () -> useCase.execute(command));
//
// assertEquals("Direct chat room already exists between these participants",
// exception.getMessage());
//
// verify(chatRoomRepository, never()).save(any());
// }
