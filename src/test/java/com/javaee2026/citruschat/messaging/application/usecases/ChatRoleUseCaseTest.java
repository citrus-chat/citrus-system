...
// import
// com.javaee2026.citruschat.messaging.application.commands.CreateChatRoleCommand;
// import
// com.javaee2026.citruschat.messaging.application.commands.CreateChatRoomCommand;
// import
// com.javaee2026.citruschat.messaging.application.commands.DeleteChatRoleCommand;
// import
// com.javaee2026.citruschat.messaging.application.commands.GetChatRolesCommand;
// import
// com.javaee2026.citruschat.messaging.application.commands.UpdateChatRoleCommand;
// import
// com.javaee2026.citruschat.messaging.application.exceptions.ChatPermissionDeniedException;
// import
// com.javaee2026.citruschat.messaging.application.exceptions.ChatRoleException;
// import
...
// import
// com.javaee2026.citruschat.messaging.infrastructure.web.dto.response.ChatRoomResponse;
// import
// com.javaee2026.citruschat.messaging.infrastructure.web.mapper.ChatRoomResponseMapper;
...
// ChatRoomId(UUID.fromString("cccccccc-0000-0000-0000-000000000001"));
// private final UserId creatorUserId = new
// UserId(UUID.fromString("cccccccc-0000-0000-0000-000000000002"));
// private final UserId requesterUserId = new
// UserId(UUID.fromString("cccccccc-0000-0000-0000-000000000003"));
// private final UserId targetUserId = new
// UserId(UUID.fromString("cccccccc-0000-0000-0000-000000000004"));
...
// private final RoleId ownerRoleId = new
// RoleId(UUID.fromString("cccccccc-0000-0000-0000-000000000008"));
// private final RoleId adminRoleId = new
// RoleId(UUID.fromString("cccccccc-0000-0000-0000-000000000009"));
// private final RoleId memberRoleId = new
// RoleId(UUID.fromString("cccccccc-0000-0000-0000-000000000010"));
// private final RoleId customRoleId = new
// RoleId(UUID.fromString("cccccccc-0000-0000-0000-000000000011"));
...
// CreateChatRoomUseCase useCase = new CreateChatRoomUseCase(chatRoomRepository,
// new ChatRoomFactory(),
...
// useCase.execute(new CreateChatRoomCommand(ChatRoomType.GROUP, "Group",
// creatorId, List.of(participantId)));
...
// .filter(participant ->
// participant.getUserId().value().equals(creatorId)).findFirst().orElseThrow();
// ChatParticipant other = room.getParticipants().stream()
// .filter(participant ->
// participant.getUserId().value().equals(participantId)).findFirst().orElseThrow();
...
// when(chatRoleRepository.findByChatRoomId(chatRoomId)).thenReturn(List.of(memberRole(),
// ownerRole()));

// var result = new GetChatRolesUseCase(chatRoomRepository, chatRoleRepository)
// .execute(new GetChatRolesCommand(chatRoomId));

// assertEquals(List.of(ownerRoleId, memberRoleId),
// result.roles().stream().map(ChatRole::getId).toList());
// }
...
// ChatRoom room = groupRoom(defaultParticipants(), defaultRoles());
// when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
// when(chatRoleRepository.existsByNameAndChatRoomId("Moderator",
// chatRoomId)).thenReturn(false);
// when(chatPermissionRepository.findAllById(Set.of(sendPermissionId.value())))
// .thenReturn(Set.of(sendPermission()));
// when(chatRoleRepository.save(any(ChatRole.class))).thenAnswer(invocation ->
// invocation.getArgument(0));

// var result = createRoleUseCase().execute(
// new CreateChatRoleCommand(chatRoomId, creatorUserId, "Moderator", 50,
...
// requesterUserId))
// .thenReturn(Optional.of(participant(requesterParticipantId, requesterUserId,
// List.of(adminRoleId))));
...
// new CreateChatRoleCommand(chatRoomId, requesterUserId, "Moderator", 50,
// List.of(sendPermissionId)));
...
// ChatPermissionDeniedException exception =
// assertThrows(ChatPermissionDeniedException.class,
// () -> createRoleUseCase().execute(new CreateChatRoleCommand(chatRoomId,
// requesterUserId, "Moderator",
...
// ChatRoom room = groupRoom(defaultParticipants(), defaultRoles());
// when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
// when(chatRoleRepository.existsByNameAndChatRoomId("Member",
// chatRoomId)).thenReturn(true);

// ChatRoleException exception = assertThrows(ChatRoleException.class, () ->
// createRoleUseCase().execute(
// new CreateChatRoleCommand(chatRoomId, creatorUserId, "Member", 10,
// List.of(sendPermissionId))));
...
// when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
// when(chatRoleRepository.findByIdAndChatRoomId(customRoleId,
// chatRoomId)).thenReturn(Optional.of(customRole));
// when(chatPermissionRepository.findAllById(Set.of(sendPermissionId.value())))
// .thenReturn(Set.of(sendPermission()));
// when(chatRoleRepository.update(any(ChatRole.class))).thenAnswer(invocation ->
// invocation.getArgument(0));

// var result = updateRoleUseCase().execute(new
// UpdateChatRoleCommand(chatRoomId, customRoleId, creatorUserId,
...
// @Test
// void
// updateRejectsRemovingRoleAdministrationPermissionsFromLastAdministrativeRole()
// {
// ChatRoom room = groupRoom(List.of(participant(creatorParticipantId,
// creatorUserId, List.of(ownerRoleId))),
// Map.of("OWNER", ownerRole()));
// when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
// when(chatRoleRepository.findByIdAndChatRoomId(ownerRoleId,
// chatRoomId)).thenReturn(Optional.of(ownerRole()));
// when(chatPermissionRepository.findAllById(Set.of(sendPermissionId.value(),
// modifyRolePermissionId.value())))
// .thenReturn(Set.of(sendPermission(), modifyRolePermission()));

// ChatRoleException exception = assertThrows(ChatRoleException.class,
// () -> updateRoleUseCase().execute(new UpdateChatRoleCommand(chatRoomId,
// ownerRoleId, creatorUserId,
...
// updateRoleUseCase().execute(new UpdateChatRoleCommand(chatRoomId,
// customRoleId, requesterUserId, "Advanced", 30,
// List.of(sendPermissionId)));
...
// when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
// when(chatRoleRepository.findByIdAndChatRoomId(customRoleId,
// chatRoomId)).thenReturn(Optional.empty());

// ChatRoleException exception = assertThrows(ChatRoleException.class,
// () -> updateRoleUseCase().execute(new UpdateChatRoleCommand(chatRoomId,
// customRoleId, creatorUserId,
// "Advanced", 30, List.of(sendPermissionId))));
...
// when(chatRoleRepository.findByIdAndChatRoomId(customRoleId, chatRoomId))
// .thenReturn(Optional.of(customRole(Set.of(sendPermission()), 20)));
// when(chatParticipantRepository.isRoleAssignedToAnyParticipant(chatRoomId,
// customRoleId)).thenReturn(false);
// when(chatRoleRepository.delete(customRoleId)).thenReturn(true);

// deleteRoleUseCase().execute(new DeleteChatRoleCommand(chatRoomId,
// customRoleId, creatorUserId, null));
...
// ChatRoleException exception = assertThrows(ChatRoleException.class, () ->
// deleteRoleUseCase()
// .execute(new DeleteChatRoleCommand(chatRoomId, customRoleId, creatorUserId,
// null)));
...
// deleteRoleUseCase().execute(new DeleteChatRoleCommand(chatRoomId,
// customRoleId, requesterUserId, null));
...
// assertThrows(ChatPermissionDeniedException.class,
// () -> deleteRoleUseCase()
// .execute(new DeleteChatRoleCommand(chatRoomId, customRoleId, requesterUserId,
// null)));
...
// ChatRoleException exception = assertThrows(ChatRoleException.class, () ->
// deleteRoleUseCase()
// .execute(new DeleteChatRoleCommand(chatRoomId, ownerRoleId, creatorUserId,
// null)));
...
// assertEquals(ErrorCode.CHAT_RULE_CONFLICT, exception.getErrorCode());
// verify(chatParticipantRepository, never()).replaceRoleForParticipants(any(),
// any(), any());
// }
...
// when(chatParticipantRepository.isRoleAssignedToAnyParticipant(chatRoomId,
// customRoleId)).thenReturn(true);
// when(chatRoleRepository.findByIdAndChatRoomId(memberRoleId,
// chatRoomId)).thenReturn(Optional.of(memberRole()));
// when(chatRoleRepository.delete(customRoleId)).thenReturn(true);

// deleteRoleUseCase().execute(new DeleteChatRoleCommand(chatRoomId,
// customRoleId, creatorUserId, memberRoleId));

// verify(chatParticipantRepository).replaceRoleForParticipants(chatRoomId,
...
// ChatRoomResponse directResponse =
// ChatRoomResponseMapper.toResponse(directRoom());
...
// return new CreateChatRoleUseCase(chatRoomRepository, chatRoleRepository,
// chatPermissionRepository,
// authorizationService());
// }

// private UpdateChatRoleUseCase updateRoleUseCase() {
// return new UpdateChatRoleUseCase(chatRoomRepository, chatRoleRepository,
// chatPermissionRepository,
// authorizationService());
// }

// private DeleteChatRoleUseCase deleteRoleUseCase() {
// return new DeleteChatRoleUseCase(chatRoomRepository, chatRoleRepository,
...
// return
// codes.stream().map(this::permissionByCode).collect(Collectors.toSet());
// }
...
// return new ChatPermission(sendPermissionId,
// ChatPermissionList.CAN_SEND_MESSAGE, "Allows sending messages");
// }

// private ChatPermission createRolePermission() {
// return new ChatPermission(createRolePermissionId,
// ChatPermissionList.CAN_CREATE_ROLE, "Allows creating roles");
// }

// private ChatPermission modifyRolePermission() {
// return new ChatPermission(modifyRolePermissionId,
// ChatPermissionList.CAN_MODIFY_ROLE, "Allows modifying roles");
// }

// private ChatPermission deleteRolePermission() {
// return new ChatPermission(deleteRolePermissionId,
// ChatPermissionList.CAN_DELETE_ROLE, "Allows deleting roles");
// }

// private ChatRole ownerRole() {
// return role(ownerRoleId, "OWNER",
// Set.of(sendPermission(), createRolePermission(), modifyRolePermission(),
// deleteRolePermission()), 100);
// }

// private ChatRole adminRole() {
// return role(adminRoleId, "ADMIN",
// Set.of(sendPermission(), createRolePermission(), modifyRolePermission(),
// deleteRolePermission()), 80);
// }
...
// private ChatRole role(RoleId roleId, String name, Set<ChatPermission>
// permissions, int priority) {
// return ChatRole.reconstitute(roleId, chatRoomId, permissions, name, priority,
// Instant.now());
// }

// private Map<String, ChatRole> defaultRoles() {
// return Map.of("OWNER", ownerRole(), "ADMIN", adminRole(), "MEMBER",
// memberRole());
// }

// private ChatRoom roomWithCustomRole() {
// return groupRoom(defaultParticipants(), Map.of("OWNER", ownerRole(), "ADMIN",
// adminRole(), "MEMBER",
...
// return groupRoom(
// List.of(participant(creatorParticipantId, creatorUserId,
// List.of(ownerRoleId)),
// participant(targetParticipantId, targetUserId, List.of(customRoleId))),
// Map.of("OWNER", ownerRole(), "ADMIN", adminRole(), "MEMBER", memberRole(),
// "CUSTOM",
// customRole(Set.of(sendPermission()), 20)));
// }

// private List<ChatParticipant> defaultParticipants() {
// return List.of(participant(creatorParticipantId, creatorUserId,
// List.of(ownerRoleId)),
...
// private ChatParticipant participant(ParticipantId participantId, UserId
// userId, List<RoleId> roleIds) {
// return ChatParticipant.reconstitute(participantId, chatRoomId, userId,
// roleIds, Instant.now(), null, null);
// }

// private ChatRoom groupRoom(List<ChatParticipant> participants, Map<String,
// ChatRole> roles) {
// return new ChatRoom(chatRoomId, ChatRoomType.GROUP, "Group", creatorUserId,
// participants, roles, Instant.now(),
// null, null);
// }

// private ChatRoom directRoom() {
// return new ChatRoom(chatRoomId, ChatRoomType.DIRECT, "Direct", creatorUserId,
// defaultParticipants(),
