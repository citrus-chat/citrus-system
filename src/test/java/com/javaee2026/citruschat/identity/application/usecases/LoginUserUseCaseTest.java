...
class LoginUserUseCaseTest {
	...
	void setUp() {
		...
		loginUserUseCase = new LoginUserUseCase(uploadPreKeysUseCase, userRepository, passwordEncoder, jwtService,
				registerOrRefreshUserDeviceUseCase);
	}

	private LoginCommand createLoginCommand(String email, String password) {
		return new LoginCommand(email, password, null, "Chrome on Windows", DeviceType.WEB, "identity-key",
				new SignedPreKeyRequest(1, "signed-public-key", "signature"),
				List.of(new OneTimePreKeyRequest(1, "prekey-1"), new OneTimePreKeyRequest(2, "prekey-2")));
	}
	...
	void shouldLoginSuccessfullyWhenCredentialsAreValid() {
		...
		when(uploadPreKeysUseCase.execute(any(UploadPreKeysCommand.class))).thenReturn(new UploadPreKeysResult(2));
		...
		verify(uploadPreKeysUseCase).execute(argThat(keysCommand -> keysCommand.deviceId().equals(DEVICE_ID)
				&& keysCommand.publicIdentityKey().equals("identity-key") && keysCommand.signedPreKey().keyId() == 1
	...
	void shouldThrowInvalidCredentialsWhenPasswordIsInvalid() {
		...
		LoginCommand command = createLoginCommand("test@gmail.com", "wrong-password");
	...
	void shouldRegisterDeviceBeforeUploadingPreKeys() {
		...
		when(jwtService.generateToken(anyString(), anyString(), anyString(), anyString())).thenReturn("jwt-token");

		when(jwtService.getExpirationInSeconds()).thenReturn(86400L);

		loginUserUseCase.execute(command);

		InOrder inOrder = inOrder(registerOrRefreshUserDeviceUseCase, uploadPreKeysUseCase, jwtService);
		...
		inOrder.verify(jwtService).generateToken(anyString(), anyString(), anyString(), anyString());
	}
}
