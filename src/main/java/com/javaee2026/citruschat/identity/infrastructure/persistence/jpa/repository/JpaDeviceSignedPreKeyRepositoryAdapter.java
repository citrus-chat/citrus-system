...
public class JpaDeviceSignedPreKeyRepositoryAdapter implements IDeviceSignedPreKeyRepository {
	...
	public JpaDeviceSignedPreKeyRepositoryAdapter(SpringDataDeviceSignedPreKeyRepository repository,
	...
	public List<DeviceSignedPreKey> findByDeviceId(DeviceId deviceId) {
		return repository.findByIdDeviceId(deviceId.value()).stream().map(mapper::toDomain).toList();
	}

	@Override
	public Optional<DeviceSignedPreKey> findActiveByDeviceId(DeviceId deviceId, Instant now) {
		return repository.findFirstByIdDeviceIdAndExpiresAtAfterOrderByCreatedAtDesc(deviceId.value(), now)
				.map(mapper::toDomain);
	}

	@Override
	public Optional<DeviceSignedPreKey> findByDeviceIdAndKeyId(DeviceId deviceId, int keyId) {
		return repository.findByIdDeviceIdAndIdKeyId(deviceId.value(), keyId).map(mapper::toDomain);
	}
