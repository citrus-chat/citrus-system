...
public final class UserProfileWebMapper {
	...
	public static UpdateUserProfileCommand toCommand(UUID userId, UpdateUserProfileRequest req) {
		return new UpdateUserProfileCommand(userId, req.username(), req.description(), req.privacy(), req.showPhone(),
				req.showEmail(), req.showStatus(), req.showDescription(), req.allowGroupInvites());
	}

	public static UserProfileResponse toResponse(UserProfileResult result) {
		return new UserProfileResponse(result.userId(), result.username(), result.avatarUrl(), result.description(),
				result.privacy(), result.showPhone(), result.showEmail(), result.showStatus(), result.showDescription(),
