...
public class GetUserProfileUseCase {
	...
	public GetUserProfileUseCase(IUserProfileRepository userProfileRepository, IUserRepository userRepository) {
	...
	public UserProfileResult execute(UUID userId) {
		...
		UserProfileJpaEntity profile = userProfileRepository.findByUserId(userId).orElseGet(() -> {
		...
		return new UserProfileResult(userId, user.getUsername().getValue(), user.getAvatarUrl(),

				profile.getDescription(), profile.getPrivacy(),

				profile.isShowPhone(), profile.isShowEmail(), profile.isShowStatus(), profile.isShowDescription(),
